...
ESSColors <- c("#ef3e42", "#ef7f1f", "#823f7a", "#9fca60", "#37afc3", "#c07396", "#48ad96", "#3183aa")
...
Inwer <- read.dta(params$intFile, convert.factors = F, convert.dates = F)

# Main Dataset
Main <- read.dta(params$mainFile, convert.factors = F, convert.dates = F)
...
# All face-to-face cases in Main
Mainf2f <- Main %>% filter(mode %in% c(1, 2))

# Filter out cases without intnum
Main <- Main %>% filter(mode %in% c(1, 2) & !is.na(intnum))
```
...
Three areas of analysis have been defined in this report. The first section presents the results on the analysis of the timestamps from the interview. The second section focuses on item nonresponse. The third section investigates response patterns of respondents and how these related to allocations within interviewers. 
...
For the interim dataset, quality benchmarks have been defined with a low sensitivity, meaning that is expected that higher number of cases will be flagged without an actual problem having occurred (false positives). The flag aim is to raise attention about possible issues for further investigation, instead of reporting the existence of an issue _per se_. 

## Evaluation of Results

_Figures, Tables and Detailed Tables in Annex Folder_

In the report, results are presented in figures and table to provide a quick overview of the issues. Problematic cases or interviewer are usually highlighted. However, it is not always possible to provide a adaquate overview of all issues for all countries. Therefore, indicators are saved into the Annex folder together with the report for further invatigation. 

_Adjustment of Quality Benchmarks_

Adjustment of the quality benchmarks might be necessary to better tune sensibility to country-specific context or to conduct a more focused investigation of issues.

## Further Investigation

All flags provided by this report are granted further consideration and investigation with the goal of gaining understanding about the causes of the issues. The steps and efforts will depend on the type of flag and the related level of concern. There are two ways to adjust the quality benchmarks used as default for this report: (1) manipulating the csv files stored in the “Annex” folder along the report with the indicators for the desired value, or (2) modifying the R-Markdown syntax provided in the tool under the “tool_file” folder (“Analysis_InterimD_R12-sav.Rmd”).
...
# Interviewers in `r thisCountry`

The data provided to the tool contained a total of `r length(Mainf2f$idno)` face-to-face or video interviews, of which `r length(Main$idno)` contained a valid interviewer number. It is assumed that all interviews have been conducted by an interviewer. Only cases with a valid interviewer number are analyzed in this report.

A total of `r length(unique(Mainf2f$intnum))` interviewers have at least one completed interview. Before interpreting the quality indicators in this report, it is important to bear in mind the number of interviews each interviewer has completed.  Table \ref{tab:table_nperm} provides a descriptive summary of the interviews per interviewer while figure \ref{fig:nperm_plot} visualizes its distribution. The distribution of cases provide a rough picture of the fieldwork activities and an indication of the robustness of the results in this report. 
...
Main$fulldate <- dmy_hms(Main$inwds)
# fulldatetime for start and end of interview
Main$start <- dmy_hms(Main$inwds)
Main$end <- dmy_hms(Main$einwe)
```
...
The general recommendation here is to go back to the survey agency (or fieldwork coordinator or interviewer in case of an in-house survey) and have them check these results with the interviewer in question. Especially in the case of interview duration, other indicators like item nonresponse (see next sections) should be taken into consideration.
...
### Unusual interview times {#sec:time}
...
ggplot(dftime_120, aes(timebtwints, as.factor(intnum))) +
          ...
          scale_colour_manual(values = c("FALSE" = "grey",
                                         "TRUE" = ESSColors[2])) +
          scale_x_continuous(breaks = seq(0, max(dftime_120$timebtwints), 15)) + 
...
inc.mis.all <- Main %>%  
  ...
  group_by(intnum) %>%
  summarize(num.mis = sum(hinctnta %in% c(77, 88, 99)), num.int = n()) %>%
  mutate(prop.mis = num.mis / num.int)
```
...
## item nonresponse for Module E {#sec:nr_moduleE}

In addition to studying item nonresponse for a sensitive question like income, we additionally aim our attention at the patterns of nonresponse in a module of less sensitive questions, in this case, *Module E* on the Human Values Scale (20 items). Again, we first give an overview of item nonresponse on the items in *Module E* in `r thisCountry`, and in the second part, we zoom in on unusual properties.

For an overview, we first check the percentages of nonresponse on the items in *Module E*. Secondly, we summarize the proportion of item nonresponse on items in *Module E* of all interviews conducted by the same interviewer. The proportion of item nonresponse $p$ on items in *Module E* for each interviewer $j$ is calculated according to Equation \ref{eq:nr_income}. Subsequently, for each interviewer, we calculate the mean proportion of item nonresponse over these 20 items.

```{r nr_moduleE, error=T}
# Selection of variables module Human Values
hvc <- c("ipdopin", "ipstst",  "ipplea",  "ipnpea",  "ipbesuc", "ipetrju",
         "ippoco",  "iphumb",  "ipprenv", "ipnhum",  "ipnexp",  "iphppcl",
         "ipwlth",  "ipprsaf", "iptlppl", "ipnvreg", "ipmadec", "ipftrad",
         "ippconf", "ipacrec")
...
for(i in hvc){
  perc.nonr[i, ] <- sum(Main[[i]]==7|Main[[i]]==8|Main[[i]]==9)/nrow(Main)*100 
}
...
prop_mis <- function(x){
  ...
  if("8" %in% colnames(tab)==F) { 
    ...
    if("9" %in% colnames(tab)==F) { 
      tab <- cbind(tab, 0)
      colnames(tab)[colnames(tab)==""] <- 9
  }
  ...
  colnames(tabout) <- c("_id", "_nr.int", "_refused", "_dk", "_tot", "_prop.ref", "_prop.dk", "prop.na", "_prop.tot")
  return(tabout)
}
...
prop.total <- select(hvs.mis, ipdopin._nr.int, ends_with("_prop.tot"))
prop.total <- prop.total %>%
  tibble::rownames_to_column(., var = "intnum") %>%
  dplyr::rename(num.int = ipdopin._nr.int)
...
Figure \ref{fig:plot_nr_moduleE} shows the distribution of the average proportion of item nonresponse for all 20 items for each interviewer. It is calculated according to Equation \ref{eq:nr_income} separately for all interviewers and interviewers with at least 10 interviews.

Overall, we can expect the proportions of nonresponse per interviewer to be relatively low, but there might be some exceptions. Looking at all interviewers, we can expect some extreme proportions for interviewers who conducted only one or a few interviews. For a better overview, the distribution of interviewers with at least 10 interviews should therefore also be considered.

```{r plot_nr_moduleE, fig.height = 5, fig.width = 7, fig.cap = paste("\\label{fig:plot_nr_moduleE} Mean proportion of item nonresponse on items in Module E",  linebreak, "Note: The red line indicates the mean of the proportion. The blue line indicates the median."), fig.scap="Proportion of average item nonresponse on items in Module E", error=T}
...
Figure \ref{fig:plot_nr_moduleE_unusual} gives an overview of outliers in proportion  of nonresponse, both for the complete sample and for the subset of interviewers who conducted 10 interviews or more. For more details, see table in the annex folder with the proportion of item nonresponse per interviewer on items in Module E ("Prop item nonresponse module E.csv"). We particularly recommend looking at interviewers who have conducted 10 or more interviews and had a proportion of nonresponse larger than 50%. Very little or no nonresponse might also be unusual and worthy of our attention. Those interviewers might be investigated more, but should not be flagged solely on this data.

```{r nr_moduleE_annex, error=T}
Prop_nr_moduleE <- prop.total %>% 
  filter(group == "All interviewers") %>% 
  select(intnum, meanmis, num.int)
Prop_nr_moduleE <- Prop_nr_moduleE[, c("intnum", "meanmis", "num.int")]

write.table(Prop_nr_moduleE,
            "Annex/Prop item nonresponse module E.csv", sep = ";", row.names = F)
```

```{r nr_moduleE_unusual_plot, fig.height = 9, fig.width = 7, fig.cap = paste("\\label{fig:plot_nr_moduleE_unusual} Mean proportion of item nonresponse on items in Module E with labelled outliers",  linebreak, "Note: The squares represent the mean of the distribution. Interviewers that are  > 1.5*IQR from the borders of the box are defined as outliers."), fig.scap="Mean proportion of item nonresponse on items in Module E with labelled outliers", error=T}
...
try(for (i in 1:max(Nchar)) {
  ...
  not_applicable <- as.numeric(paste(rep(6, max(Help)), collapse = ""))
  refusal <- as.numeric(paste(rep(7, max(Help)), collapse = ""))
  dont_know <- as.numeric(paste(rep(8, max(Help)), collapse = ""))
  no_answer <- as.numeric(paste(rep(9, max(Help)), collapse = ""))
  ...
  Main_mis <- Main_mis %>% replace_with_na_at(.vars = names(Help),
                                              condition = ~.x == refusal)
  Main_mis <- Main_mis %>% replace_with_na_at(.vars = names(Help),
                                              condition = ~.x == dont_know)
  Main_mis <- Main_mis %>% replace_with_na_at(.vars = names(Help),
                                              condition = ~.x == no_answer)
}, silent = T)
...
In our analysis approach, we measure non-differentiation by calculating the Mulligan Score [@mulligan2001]. It measures the average mean square root of the absolute difference of all answers within an item block for each respondent. For easier interpretation, we have normalized the reference scale and reversed the score, so that 0 indicates the least non-differentiation and 1 indicates the most non-differentiation. We define a Mulligan Score 1.5 times out of the interquartile range (IQR) as improbable.

**Non-differentiation results for Module E** 

```{r nondiff, error=T}
# Module E
# define 7 (Refusal), 8 (Don't know), and 9 (No answer) as Na
Module_E <- select(Main, ipdopin:ipacrec)
Main_nondiffE <- Main %>%
  select(c(idno, intnum, ipdopin:ipacrec)) %>%
  replace_with_na_at(.vars = names(Module_E),
                     condition = ~.x %in% c(7, 8, 9))
# normalization (x-min(x))/(max(x)-min(x)); min(x)=1; max(x)=6
Main_nondiffE <- mutate(Main_nondiffE,
                        h1 = ((ipdopin-1)/(6-1)), h2 = ((ipstst-1)/(6-1)), h3 = ((ipplea-1)/(6-1)), h4 = ((ipnpea-1)/(6-1)),
                        h5 = ((ipbesuc-1)/(6-1)), h6 = ((ipetrju-1)/(6-1)), h7 = ((ippoco-1)/(6-1)), h8 = ((iphumb-1)/(6-1)),
                        h9 = ((ipprenv-1)/(6-1)), h10 = ((ipnhum-1)/(6-1)), h11 = ((ipnexp-1)/(6-1)), h12 = ((iphppcl-1)/(6-1)),
                        h13 = ((ipwlth-1)/(6-1)), h14 = ((ipprsaf-1)/(6-1)), h15 = ((iptlppl-1)/(6-1)), h16 = ((ipnvreg-1)/(6-1)),
                        h17 = ((ipmadec-1)/(6-1)), h18 = ((ipftrad-1)/(6-1)), h19 = ((ippconf-1)/(6-1)), h20 = ((ipacrec-1)/(6-1)))
...
Main_nondiffE <- mutate(Main_nondiffE,
                        h_mrp= ((sqrt(abs(h1-h2)) + sqrt(abs(h1-h3)) + sqrt(abs(h1-h4)) + sqrt(abs(h1-h5)) + sqrt(abs(h1-h6)) +
                                   ...
                                   sqrt(abs(h1-h17)) + sqrt(abs(h1-h18)) + sqrt(abs(h1-h19)) + sqrt(abs(h1-h20)) +
                                   ...
                                   sqrt(abs(h2-h17)) + sqrt(abs(h2-h18)) + sqrt(abs(h2-h19)) + sqrt(abs(h2-h20)) +
                                   ...
                                   sqrt(abs(h3-h17)) + sqrt(abs(h3-h18)) + sqrt(abs(h3-h19)) + sqrt(abs(h3-h20)) +
                                   ...
                                   sqrt(abs(h4-h17)) + sqrt(abs(h4-h18)) + sqrt(abs(h4-h19)) + sqrt(abs(h4-h20)) +
                                   ...
                                   sqrt(abs(h5-h17)) + sqrt(abs(h5-h18)) + sqrt(abs(h5-h19)) + sqrt(abs(h5-h20)) +
                                   ...
                                   sqrt(abs(h6-h17)) + sqrt(abs(h6-h18)) + sqrt(abs(h6-h19)) + sqrt(abs(h6-h20)) +
                                   ...
                                   sqrt(abs(h7-h17)) + sqrt(abs(h7-h18)) + sqrt(abs(h7-h19)) + sqrt(abs(h7-h20)) +
                                   ...
                                   sqrt(abs(h8-h17)) + sqrt(abs(h8-h18)) + sqrt(abs(h8-h19)) + sqrt(abs(h8-h20)) +
                                   
                                   sqrt(abs(h9-h10)) + sqrt(abs(h9-h11)) +
                                   sqrt(abs(h9-h12)) + sqrt(abs(h9-h13)) + sqrt(abs(h9-h14)) + sqrt(abs(h9-h15)) + sqrt(abs(h9-h16)) +
                                   sqrt(abs(h9-h17)) + sqrt(abs(h9-h18)) + sqrt(abs(h9-h19)) + sqrt(abs(h9-h20)) +
                                   
                                   sqrt(abs(h10-h11)) +
                                   sqrt(abs(h10-h12)) + sqrt(abs(h10-h13)) + sqrt(abs(h10-h14)) + sqrt(abs(h10-h15)) + sqrt(abs(h10-h16)) +
                                   sqrt(abs(h10-h17)) + sqrt(abs(h10-h18)) + sqrt(abs(h10-h19)) + sqrt(abs(h10-h20)) +
                                   
                                   sqrt(abs(h11-h12)) + sqrt(abs(h11-h13)) + sqrt(abs(h11-h14)) + sqrt(abs(h11-h15)) + sqrt(abs(h11-h16)) +
                                   sqrt(abs(h11-h17)) + sqrt(abs(h11-h18)) + sqrt(abs(h11-h19)) + sqrt(abs(h11-h20)) +
                                   
                                   sqrt(abs(h12-h13)) + sqrt(abs(h12-h14)) + sqrt(abs(h12-h15)) + sqrt(abs(h12-h16)) +
                                   sqrt(abs(h12-h17)) + sqrt(abs(h12-h18)) + sqrt(abs(h12-h19)) + sqrt(abs(h12-h20)) +
                                   
                                   sqrt(abs(h13-h14)) + sqrt(abs(h13-h15)) + sqrt(abs(h13-h16)) +
                                   sqrt(abs(h13-h17)) + sqrt(abs(h13-h18)) + sqrt(abs(h13-h19)) + sqrt(abs(h13-h20)) +
                                   
                                   sqrt(abs(h14-h15)) + sqrt(abs(h14-h16)) +
                                   sqrt(abs(h14-h17)) + sqrt(abs(h14-h18)) + sqrt(abs(h14-h19)) + sqrt(abs(h14-h20)) +
                                   
                                   sqrt(abs(h15-h16)) +
                                   sqrt(abs(h15-h17)) + sqrt(abs(h15-h18)) + sqrt(abs(h15-h19)) + sqrt(abs(h15-h20)) +
                                   
                                   sqrt(abs(h16-h17)) + sqrt(abs(h16-h18)) + sqrt(abs(h16-h19)) + sqrt(abs(h16-h20)) +
                                   
                                   sqrt(abs(h17-h18)) + sqrt(abs(h17-h19)) + sqrt(abs(h17-h20)) +
                                   
                                   sqrt(abs(h18-h19)) + sqrt(abs(h18-h20)) +
                                   
                                   sqrt(abs(h19-h20)))/190))
                        
# Normalization and reverse coding of Mulligan Score (Score between 0...least straightlining and 1...most straighlining)
Main_nondiffE <- mutate(Main_nondiffE, 
                        h_mulligan = ((h_mrp - max(Main_nondiffE$h_mrp, na.rm = T))/
                                        ((min(Main_nondiffE$h_mrp, na.rm = T)-max(Main_nondiffE$h_mrp, na.rm = T)))))

# average Mulligan Score for each interviewer
Mulligan_E_all <- Main_nondiffE %>%
  group_by(intnum) %>%
  summarise(avg=mean(h_mulligan, na.rm = TRUE)) %>%
  mutate(group = "All interviewers")
# average Mulligan Score for interviewers with at least 4 interviews
Mulligan_E_4 <- Main_nondiffE %>%
  group_by(intnum) %>%
  filter(n() > 3) %>%
  summarise(avg=mean(h_mulligan, na.rm = TRUE)) %>%
  mutate(group = "Interviewers with at least 4 interviews")

Mulligan <- bind_rows(Mulligan_E_all, Mulligan_E_4) %>%
