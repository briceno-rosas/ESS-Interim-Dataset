...
library(lubridate)
library(wesanderson)
#library(colortools) # adjacent works #removed
...
ESSColors <- c("#E83352", "#E86F33", "#E833AD", "#ACE833", "#33E8C9", "#6F33E8", "#249E82", "#00407A")
...
Main$end_e <- as_datetime(Main$einwe, origin = "1582-10-14")
...
Main_H <- dplyr::select(df_duration,c(idno, intnum, end_f, end_h, ipcrtiva:impfuna))
# number of applicable items q_H
Main_H <- Main_H %>% mutate(q_H = rowSums(!is.na(select(., -c(intnum, idno, end_f, end_h)))),
                            q_Hvalid = rowSums(!is.na(select(., -c(intnum, idno, end_f, end_h))) &
                                                 select(., -c(intnum, idno, end_f, end_h)) < 7))
# time needed to complete module t_H
Main_H <- Main_H %>% mutate(t_H = as.numeric(end_h - end_f)/60) %>%
  filter(t_H != 0)
# speed over a the module v_H
Main_H <- Main_H %>% mutate(v_H = q_H/t_H,
                            v_Hvalid = q_Hvalid/t_H)
# average interviewer speed for all interviewers
Speed_H_all <- Main_H %>%
  group_by(intnum) %>%
  summarise(avg_speed=mean(v_Hvalid)) %>%
...
write.table(Main_H[,c("idno", "intnum", "v_Hvalid")] %>% dplyr::rename(speed = v_Hvalid), 
...
write.table(Mulligan_H_all[,c("intnum", "avg")] %>% dplyr::rename(avg_mulligan = avg),
            "Annex/Non differentiation in module H per interviewer.csv", sep = ";", row.names = F)
