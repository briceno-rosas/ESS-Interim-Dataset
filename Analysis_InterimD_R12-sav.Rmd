...
link-citations: yes
params:
  mainFile: "DATA_R11/ESS11_DATA_MAIN_QUESTIONNAIRE_TIMESTAMPS_TEST.sav"
  intFile: "DATA_R11/ESS11_DATA_INTERVIEWER_QUESTIONNAIRE_TEST.sav"
...
# Selection of variables module Euman Values
  hvc <- c("ipcrtiva", "impricha", "ipeqopta", "ipshabta", "impsafea", "impdiffa", 
...
# create matrix to store percentage nonresponse per question
  ...
  for(i in hvc){
    perc.nonr[i, ] <- sum(Main[[i]]==77|Main[[i]]==88|Main[[i]]==99)/nrow(Main)*100 
  }
...
# write function that gives missings and proportion of missings   
  ...
  if("77" %in% colnames(tab)==F) {
      tab <- cbind(tab, 0)
      colnames(tab)[colnames(tab)==""] <- 77
  } 
    
  if("88" %in% colnames(tab)==F) { 
      tab <- cbind(tab, 0)
      colnames(tab)[colnames(tab)==""] <- 88
  }
  
    if("99" %in% colnames(tab)==F) { 
      tab <- cbind(tab, 0)
      colnames(tab)[colnames(tab)==""] <- 99
  }
  
    mis <- tab[,c("77", "88", "99")]
    mis.tot <-  rowSums(mis)
    int <- rowSums(tab)
    prop.ref <- tab[,c("77")]/int
    prop.dk <- tab[,c("88")]/int
    prop.na <- tab[,c("99")]/int
...
# Descriptives for proportion missings per interviewer - per variable  
  prop.total <- select(hvs.mis, ipcrtiva._nr.int, ends_with("_prop.tot"))
  prop.total <- prop.total %>%
    tibble::rownames_to_column(., var = "intnum") %>%
    dplyr::rename(num.int = ipcrtiva._nr.int)
...
# special case Module E
Modul_H <- select(Main, ipcrtiva:impfuna)
Main_misH <- Main %>%
  select(c(idno, intnum, ipcrtiva:impfuna)) %>%
...
Main_nondiffE <- mutate(Main_nondiffE,
                        h1 = ((ipcrtiva-1)/(6-1)), h2 = ((impricha-1)/(6-1)), h3 = ((ipeqopta-1)/(6-1)), h4 = ((ipshabta-1)/(6-1)),
                        h5 = ((impsafea-1)/(6-1)), h6 = ((impdiffa-1)/(6-1)), h7 = ((ipfrulea-1)/(6-1)), h8 = ((ipudrsta-1)/(6-1)),
                        h9 = ((ipmodsta-1)/(6-1)), h10 = ((ipgdtima-1)/(6-1)), h11 = ((impfreea-1)/(6-1)), h12 = ((iphlppla-1)/(6-1)),
                        h13 = ((ipsucesa-1)/(6-1)), h14 = ((ipstrgva-1)/(6-1)), h15 = ((ipadvnta-1)/(6-1)), h16 = ((ipbhprpa-1)/(6-1)),
                        h17 = ((iprspota-1)/(6-1)), h18 = ((iplylfra-1)/(6-1)), h19 = ((impenva-1)/(6-1)), h20 = ((imptrada-1)/(6-1)),
                        h21 = ((impfuna-1)/(6-1)))
