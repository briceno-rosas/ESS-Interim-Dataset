...
link-citations: yes
params:
  mainFile: "DATA_FMS/cases_BE.csv"
  intFile: "DATA_FMS/cases_BE.csv"
...
# Overview of fieldwork in `r thisCountry` {#sec:fiedlworkoverview}
...
#CF$nattempts <- CF$case_ntimes_visited #Old R10 / Variable removed from analysis
#CF$nvisits <- CF$case_ntimes_visited_face #Old R10
CF$nvisits <- CF$case_ntimes_visited #In R11 visited and contact attempt assumed as one
...
CF_main <- CF[,c("cntry", "Country", "idno", "intnum", "finalcode", "finalcodeaggregated", 
                         "dateresult","lastattempt.mode", "nvisits")]
names(CF)
```
...
It should be noted that the Round 11 version of the FMS Upload does not differentiated between personal visits and contact attempts without a personal visit.
...
Figure \ref{fig:npv_plot} shows how many visits have been made to the sample units so far. 
...
Figure \ref{fig:pv2nc_plot} shows the number of visits conducted to cases currently considered as non‐ contacts. Non‐contacts with less than four individuals are shown in grey.
...
Currently `r length(unique(CF$intnum))` interviewers have sample units assigned to them in the FMS Upload Portal. `r length(unique(CF[CF$nvisits>0,]$intnum))` interviewers carried out at least one contact attempt on any of their assigned cases. 
...
#IwerQI1 <- full_join(IwerQI1, IwerNCless4ca) #Removed R11
