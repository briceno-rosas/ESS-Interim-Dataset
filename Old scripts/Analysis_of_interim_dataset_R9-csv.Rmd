...
link-citations: yes
params:
  mainFile: "Deposit Data R9/ESS9_DE_ms_v1.sav"
  intFile: "Deposit Data R9/ESS9_DE_i_v1.sav"
---
...
Inwer <- foreign::read.csv2(params$intFile, dec=".", stringsAsFactors=F)

# Main Dataset
Main <- foreign::read.csv2(params$mainFile, dec=".", stringsAsFactors=F)

Inwer_ID <- dplyr::select(Inwer, "intnum", "idno", "cntry")
Main <- dplyr::left_join(Main, Inwer_ID, by = c("idno", "cntry"))
```

```{r countrynames_rounds}
# this Country
...
Main$fulldate <- with(Main, paste(inwyys,inwmms,inwdds, sep="-")) %>% as.Date()
# fulldatetime for start and end of interview
Main$start <- with(Main, ISOdatetime(inwyys, inwmms, inwdds, inwshh, inwsmm, "00"))
Main$end <- with(Main, ISOdatetime(inwyye, inwmme, inwdde, inwehh, inwemm, "00"))
# fulldatetime for modules
Main$end_a <- with(Main, ISOdatetime(inwyye, inwmme, inwdde, ainwehh, ainwemm, "00"))
Main$end_b <- with(Main, ISOdatetime(inwyye, inwmme, inwdde, binwehh, binwemm, "00"))
Main$end_c <- with(Main, ISOdatetime(inwyye, inwmme, inwdde, cinwehh, cinwemm, "00"))
Main$end_d <- with(Main, ISOdatetime(inwyye, inwmme, inwdde, dinwehh, dinwemm, "00"))
Main$end_f <- with(Main, ISOdatetime(inwyye, inwmme, inwdde, finwehh, finwemm, "00"))
Main$end_g <- with(Main, ISOdatetime(inwyye, inwmme, inwdde, ginwehh, ginwemm, "00"))
Main$end_h <- with(Main, ISOdatetime(inwyye, inwmme, inwdde, hinwehh, hinwemm, "00"))
Main$end_i <- with(Main, ISOdatetime(inwyye, inwmme, inwdde, iinwehh, iinwemm, "00"))
```
...
```{r duration, error=T}
# duration in minutes
        Main$duration <- as.numeric(Main$end - Main$start)
      ...
      # order intnum by mean of duration per interviewer in filtered data set
        df_duration$intnum <- factor(df_duration$intnum, levels = unique(df_duration$intnum[order(df_duration$duration_mean)]))
...
# Module H without imrobable interview durations
...
```{r speed_H_plot, fig.height = 9, fig.width = 7, fig.cap = paste("\\label{fig:plot_speed_H} Average interviewing speed for Module H",  linebreak, "Note: Interviewing speed is defined as items answered per minute. The squares represent the mean of the distribution. Interviewers that are  > 1.5*IQR from the borders of the box are labelled."), fig.scap="Average interviewing speed for Module H"}
...
The (date and) time in which the interviews were conducted is another indicator that has proven useful for detecting undesirable interviewer behaviour or other issues in the field. It can inform about compliance with contacting and interviewing protocol, performance in achieving the respondent???s co??? operation, as well as error in the recording of timestamps. First, we look at interviews with an over??? lapping interview time conducted by the same interviewer. Second, the time of the day in which interviews were conducted is checked for unlikely interview hours (e.g., the middle of the night). Third, we look at the number of interviews conducted on the same day by the same interviewer. Lastly, the time between consecutive interviews of the same interviewer is analysed and its plausibility assessed.
...
In `r thisCountry`, unlikely interview times were `r ifelse(length(inw_times)==0, "not detected for any interviewer", paste("detected for", length(inw_times), "case(s)"))` `r ifelse(length(unique(inw_times))==0, "", paste(c("(intnum =", unique(inw_times), ")"), collapse= " ") )`.

### Number of interviews on the same day by a single interviewer {#sec:number_int}

The maximum number of interviews of a single day by a single interviewer is an indicator of peak performance by an interviewer. Interviewers should organize and schedule their contact attempts and appointments in such a way that allows them to work as efficiently as possible. However, in some cases, this peak performance indicator can be related to non-compliance with contacting, selection or interviewing protocols. Therefore, national teams are recommended to closely monitor the work of interviewers with an extremely high number of completed interviews within a single day. 

```{r number_int, error=T}
# n>3 interviews on the same day, indicator: ninw_day
Ninw_day <- Main %>% 
  group_by(intnum, fulldate) %>% 
  dplyr::count(.) %>%
  filter(n>3)
...
if("77" %in% colnames(inc.mis)==F) {
    inc.mis <- cbind(inc.mis, 0)
    colnames(inc.mis)[colnames(inc.mis)==""] <- 77
} 

if("88" %in% colnames(inc.mis)==F) { 
    inc.mis <- cbind(inc.mis, 0)
    colnames(inc.mis)[colnames(inc.mis)==""] <- 88
} 

if("99" %in% colnames(inc.mis)==F){
    inc.mis <- cbind(inc.mis, 0)
    colnames(inc.mis)[colnames(inc.mis)==""] <- 99
}
...
```{r nr_income_plot1, fig.height = 5, fig.width = 7, fig.cap = paste("\\label{fig:plot1_nr_income} Proportion of item-nonresponse in income per interviewer",  linebreak, "Note: The red line indicates the mean of the proportion. The blue line indicates the median."), fig.scap= "Proportion of item-nonresponse in income"}
...
if("9" %in% colnames(tab)==F){
    ...
    assign("tabout", cbind(rownames(mis), int, mis, mis.tot, prop.ref, prop.dk, prop.na, prop.tot))
    colnames(tabout) <- c("_id", "_nr.int", "_refused", "_dk", "_na", "_tot", "_prop.ref", "_prop.dk", "_prop.na", "_prop.tot")
    return(tabout)
    }
...
```{r nr_moduleH_unusual_plot, fig.height = 9, fig.width = 7, fig.cap = paste("\\label{fig:plot_nr_moduleH_unusual} Mean proportion of item-nonresponse on items in Module H with labelled outliers",  linebreak, "Note: The squares represent the mean of the distribution. Interviewers that are  > 1.5*IQR from the borders of the box are defined as outliers."), fig.scap="Mean proportion of item-nonresponse on items in Module H with labelled outliers"}
...
if("testhi" %in% names(Main)==F) {
   Main_mis <- Main %>%
  select(idno, intnum,
         nwspol:pcmpinj) %>% # Modul A to Modul G
  select_if(is.numeric)
} else { 
Main_mis <- Main %>%
  select(idno, intnum,
         nwspol:pcmpinj, # Modul A to Modul G
         # ipcrtiv:impfun, # Modul H
         testhi1:testhi9) %>% # Modul I
  select_if(is.numeric)
}
...
```{r latentvar_plot1, fig.height = 9, fig.width = 7, fig.cap = paste("\\label{fig:plot1_latentvar} Mean of component scores across interviews",  linebreak, "Note: Interviewers whose mean score exceeds ?? 2 standard deviations from the mean across interviewers are highlighted."), fig.scap="Mean of component scores across interviews"}
...
```{r latentvar_plot2, fig.height = 9, fig.width = 7, fig.cap = paste("\\label{fig:plot2_latentvar} Standard deviation of component scores across interviews", "Note: Interviewers whose mean score exceeds ?? 2 standard deviations from the mean standard deviation across interviewers are highlighted."), fig.scap="Standard deviation of component scores across interviews"}
